...
public sealed class LicenseForm : Form
{
    ...
    private void OnClickBtnActivate(object sendor, EventArgs evtObj)
    {
        ...
        array[0] = tbUserName.Text.Trim();
        array[1] = tbSerialKey.Text.Trim();
        string[] contents = array;
        string startupPath = Application.StartupPath;
        string path2 = Path.Combine(startupPath, "license.dat");
        try
        {
    ...
    private void OnClickBtnClose(object sendor, EventArgs evtObj)
    {
        Process.GetCurrentProcess().Kill();
    }

    private void OnClickBtnCopy(object sendor, EventArgs evtObj)
    {
        Clipboard.SetText(tbHardwareID.Text);
    }

    private void OnLoad(object sendor, EventArgs evtObj)
    {
    ...
    private void OnEnterGroupBox1(object sendor, EventArgs evtObj)
    {
    }

    protected override void Dispose(bool bVal)
    {
        if (bVal)
        {
            if (iContainer != null)
    ...
    private void InitUI()
    {
        ...
        tbHardwareID.Size = new Size(304, 20);
        ...
        pictureBox2.Image = AppResourceManager.GetPaymentPatternBitmap();
